...
public class ReferenceInvokerCreator<T> extends ReferenceConfigBase<T> {

    public static final Logger logger = LoggerFactory.getLogger(org.apache.dubbo.config.ReferenceConfig.class);
    ...
    public ReferenceInvokerCreator(Class interfaceClass,String ... address) {
        super();
        ...
        for (String s : address) {
            RegistryConfig registryConfig = new RegistryConfig(s);
            registryConfig.refresh();
            registries.add(registryConfig);
        }
    }

    public ReferenceInvokerCreator(Reference reference) {
        super(reference);
        this.repository = ApplicationModel.getServiceRepository();
    }
    ...
    public synchronized InvokerInvocationHandler getInvokerHandler() {
        if (destroyed) {
        ...
        return new InvokerInvocationHandler(getInvoker());
    }
...
//        checkStubAndLocal(interfaceClass);
        ...
        if (!ProtocolUtils.isGeneric(generic)) {
            ...
            String[] methods = new String[interfaceClass.getDeclaredMethods().length];
            int i = 0;
            for (Method declaredMethod : interfaceClass.getDeclaredMethods()) {
                methods[i++] = declaredMethod.getName();
            }
            if (methods.length == 0) {
...
//        dispatch(new ReferenceConfigInitializedEvent(this, invoker));
    ...
    private void createProxy(Map<String, String> map) {
